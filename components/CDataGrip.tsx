  ...
  return (
    <div className="card rounded-lg w-full bg-base-100 ">
      ...
      <div className="flex space-x-2">
        <Card label={t('课程综合评分')} score={data?.overallRatting || 0} color="origin"></Card>
        <Card label={t('平均成绩')} score={`${data?.courseAverage?.score || 0}% ${data.courseAverage?.level || 0}`} color="lightOrigin"></Card>
      </div>
      <div className="flex justify-between mt-2">
        <CScoreCard type={2} title={t('内容评分')} score={data?.contentRating}></CScoreCard>
        <CScoreCard type={2} title={t('作业评分')} score={data?.homeworkRating}></CScoreCard>
        <CScoreCard type={2} title={t('考试评分')} score={data?.examRating}></CScoreCard>
      </div>
      </div>
    </div>
  );
