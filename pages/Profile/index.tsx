...
function index(props) {
    ...
    return (
      ...
      >
        <div className="flex items-center space-x-2">
          ...
          <div className="pl-10 font-bold text-brown">{t('学生认证')}</div>
        </div>
        <div className="flex flex-col items-center text-xs text-brown">
          <div>{t('30秒认证在校生身份')}</div>{' '}
          <div>{t('解锁YoUni全部功能')}</div>
        </div>
    ...
    return (
      <div className="flex justify-between p-4  relative">
        <div className="flex flex-col items-center  space-y-3">
          <MenuIcon1></MenuIcon1>
          <div className="text-xs text-[#798195]">{t('每日签到')}</div>
        </div>
        <div className="flex flex-col items-center space-y-3">
          <MenuIcon2></MenuIcon2>
          <div className="text-xs text-[#798195]">{t('评论和@')}</div>
        </div>
        <div className="flex flex-col items-center space-y-3">
          <MenuIcon3></MenuIcon3>
          <div className="text-xs text-[#798195]">{t('赞&收藏')}</div>
        </div>
      </div>
    );
  };
          ...
          <div className="grid grid-cols-4 ">
            <Link href="/Setting/account">
              <div className="flex flex-col items-center space-y-3">
                ...
                <div className="text-xs text-[#798195]">{t('账号')}</div>
              </div>
            </Link>
            <div className="flex flex-col items-center space-y-3">
              <SettingIcon2></SettingIcon2>
              <Link href="/Setting/">
                <div className="text-xs text-[#798195]">{t('草稿箱')}</div>
              </Link>
            </div>
            <div className="flex flex-col items-center space-y-3">
              <SettingIcon3></SettingIcon3>
              <div className="text-xs text-[#798195]">{t('历史')}</div>
            </div>
            <div className="flex flex-col items-center space-y-3">
              <SettingIcon4></SettingIcon4>
              <Link href="/Setting">
                <div className="text-xs text-[#798195]">{t('功能请求')}</div>
              </Link>
            </div>
          </div>
          <div className="grid grid-cols-4 ">
            <div className="flex flex-col items-center space-y-3">
              ...
              <div className="text-xs text-[#798195]">{t('邀请好友')}</div>
            </div>
            <div className="flex flex-col items-center space-y-3">
              <SettingIcon6></SettingIcon6>
              <div className="text-xs text-[#798195]">{t('客服')}</div>
            </div>
            <div className="flex flex-col items-center space-y-3">
              <SettingIcon7></SettingIcon7>
              <div className="text-xs text-[#798195]">{t('我的积分')}</div>
            </div>
            <Link href="/Setting/contact">
              <div className="flex flex-col items-center space-y-3">
                <SettingIcon8></SettingIcon8>
                <div className="text-xs text-[#798195]">{t('联系方式')}</div>
              </div>
            </Link>
          </div>
          <div className="grid grid-cols-4 ">
            <div className="flex flex-col items-center space-y-3">
              ...
              <div className="text-xs text-[#798195]">{t('加入有你')}</div>
            </div>
            <div className="flex flex-col items-center space-y-3">
              <SettingIcon10></SettingIcon10>
              <div className="text-xs text-[#798195]">{t('社团入驻')}</div>
            </div>
            <Link href="/Setting">
              <div className="flex flex-col items-center space-y-3">
                <SettingIcon11></SettingIcon11>
                <div className="text-xs text-[#798195]">{t('系统设置')}</div>
              </div>
            </Link>
          </div>
        </div>
            ...
            <>
              {PostGroupData?.data.length === 0 ? (
                <div className="flex mt-10 flex-col space-y-4 items-center justify-center ">
                  ...
                  <div className="text-xs text-[#A9B0C0]">{t('暂无关注文集')}</div>
                </div>
              ) : null}
            </>
          </div>
  ...
  const EmptyData = () => {
    const columns = [
      { text: t('帮你定位最好的教授'), value: '0' },
      { text: t('一键导入大学课表'), value: '1' },
      { text: t('便捷计算自己的GPA'), value: '2' },
      ];
      ...
      <div className="w-full h-[400px]  p-5">
        <div className="relative">
          ...
          <Picker
            ...
            value={value}
            className="h-[220px]"
            onCancel={() => Toast.info(t('点击取消按钮'))}
            swipeDuration={10}
            onConfirm={() => Toast.info(t('点击确认按钮'))}
          />
        </div>
        <button
          onClick={() => {
